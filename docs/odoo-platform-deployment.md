...
* `docker-compose.yml`
 ...
 ```
 odoo:
  image: camptocamp/sensefly_odoo:10.0.0
...
* `rancher.public.env`
 ...
 ```
 export DOMAIN_NAME=integration.sensefly.odoo.camptocamp.ch
 ...
 export AWS_BUCKETNAME=sensefly-odoo-integration
 
 export ODOO_SESSION_REDIS=1
 export ODOO_SESSION_REDIS_HOST=redis
 export ODOO_SESSION_REDIS_PREFIX=sensefly-odoo-integration
...
* `docker-compose.yml`
 ...
 letsencrypt:
   ...
   image: janeczku/rancher-letsencrypt:v0.3.0
   volumes:
     - sensefly-prod-certs:/etc/letsencrypt/production/certs
 ```
...
* `rancher.public.env`
 ...
 export AWS_BUCKETNAME=sensefly-odoo-prod
 
 export ODOO_SESSION_REDIS=1
 export ODOO_SESSION_REDIS_HOST=redis
 export ODOO_SESSION_REDIS_PREFIX=sensefly-odoo-prod
...
```
./rancher sensefly-odoo-integration up -d letsencrypt
./rancher sensefly-odoo-prod up -d letsencrypt
```
...
* Use the local `./rancher` script, example to start the containers:
  ...
  ```
  ./rancher sensefly-odoo-integration up -d
  ```
...
* Use the local `./rancher` script, example to upgrade the containers:
  ...
  ```
  ./rancher sensefly-odoo-integration up --pull --recreate --confirm-upgrade -d
  ```
